...
class ServerActions implements Runnable {
    ...
    void
            executeCommand(JsonObject data) throws IOException, UnrecoverableKeyException, GeneralSecurityException {
        ...
        if (cmd.getAsString().equals("create")) {
            ...
            if (me == null) {
                sendResult("\"result\":\" something went bad \"", null);
            } else {
                sendResult("\"result\":\"" + me.id + "\"", null);
            }
//            GenerateKeys.create(me.id);
            return;
        }

        // LIST
        if (cmd.getAsString().equals("list")) {
            System.out.println("entrou em list");
            String list;
        ...
        if (cmd.getAsString().equals("check")) {
            JsonElement name = data.get("uuid");
            ...
            if (registry.getUserString(s[1]) == null) {
                sendResult(null, "\"Dosen't exists\"");
                return;
            }
            
            sendResult(registry.getUserString(s[1]).toString(), null);
            return;
    ...
    public void run() {
        while (true) {
            try {
                ...
                Logger.getLogger(ServerActions.class.getName()).log(Level.SEVERE, null, ex);
            }
        }
